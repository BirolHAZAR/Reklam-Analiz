...
if SENTRY_DSN and not _is_runserver_reloader_parent():
    ...
    if SENTRY_STARTUP_LOG:
        print(
            f"Sentry entegrasyonu aktif "
            f"(Environment: {DJANGO_ENV})"
        )

elif not SENTRY_DSN and SENTRY_STARTUP_LOG:
    print(
        "Sentry DSN bulunamadi; "
        "hata takibi devre disi."
    )
...
os.makedirs(
    CELERYBEAT_RUNTIME_DIR,
    exist_ok=True,
)


# ============================================================
# TEMEL DJANGO AYARLARI
# ============================================================

# SECRET_KEY kesinlikle settings.py içinde tutulmaz.
# .env / Dokploy Environment Variables üzerinden gelir.
...
ALLOWED_HOSTS = config(
    'ALLOWED_HOSTS',
    default='reklamanaliz.net,www.reklamanaliz.net',
).split(',')
...
CSRF_TRUSTED_ORIGINS = [
    ...
    for origin in config(
        "CSRF_TRUSTED_ORIGINS",
        default=(
            "https://reklamanaliz.net,"
            "https://www.reklamanaliz.net"
        ),
        cast=str,
    ).split(",")
    if origin.strip()
...
# PostgreSQL
#
# ŞİFRE settings.py içinde tutulmaz.
# DB_PASSWORD sadece .env / Dokploy'dan alınır.
# ============================================================
...
ORGANIC_INSTAGRAM_PUBLISH_ENABLED = config(
    'ORGANIC_INSTAGRAM_PUBLISH_ENABLED',
    default=False,
    cast=bool,
)

ORGANIC_FACEBOOK_PUBLISH_ENABLED = config(
    'ORGANIC_FACEBOOK_PUBLISH_ENABLED',
    default=False,
    cast=bool,
)

ORGANIC_TIKTOK_PUBLISH_ENABLED = config(
    'ORGANIC_TIKTOK_PUBLISH_ENABLED',
    default=True,
    cast=bool,
)

ORGANIC_X_PUBLISH_ENABLED = config(
    'ORGANIC_X_PUBLISH_ENABLED',
    default=True,
    cast=bool,
)

ORGANIC_LINKEDIN_PUBLISH_ENABLED = config(
    'ORGANIC_LINKEDIN_PUBLISH_ENABLED',
    default=True,
    cast=bool,
)

LINKEDIN_API_VERSION = config(
...
OPENAI_USAGE_API_KEY = config(
    'OPENAI_USAGE_API_KEY',
    default=OPENAI_API_KEY,
)

OPENAI_USAGE_CYCLE_START_DATE = config(
    'OPENAI_USAGE_CYCLE_START_DATE',
    default='2026-06-26',
)
...
OPENAI_SHOPPING_VISION_MODEL = config(
    'OPENAI_SHOPPING_VISION_MODEL',
    default='gpt-5.6-sol',
)

OPENAI_SHOPPING_PROMPT_MODEL = config(
    'OPENAI_SHOPPING_PROMPT_MODEL',
    default='gpt-5.6-sol',
)

OPENAI_SHOPPING_PREFILTER_MODEL = config(
    'OPENAI_SHOPPING_PREFILTER_MODEL',
    default='gpt-5.6-luna',
)

OPENAI_SHOPPING_MATCH_MODEL = config(
    'OPENAI_SHOPPING_MATCH_MODEL',
    default='gpt-5.6-terra',
)

OPENAI_SHOPPING_QA_MODEL = config(
    'OPENAI_SHOPPING_QA_MODEL',
    default='gpt-5.6-sol',
)

OPENAI_SHOPPING_FALLBACK_MODEL = config(
    'OPENAI_SHOPPING_FALLBACK_MODEL',
    default='gpt-5.6-terra',
)

OPENAI_CREATIVE_ANALYSIS_MODEL = config(
    'OPENAI_CREATIVE_ANALYSIS_MODEL',
    default='gpt-5.6-sol',
)

OPENAI_CREATIVE_WORK_MODEL = config(
    'OPENAI_CREATIVE_WORK_MODEL',
    default='gpt-5.6-terra',
)

OPENAI_CREATIVE_QA_MODEL = config(
    'OPENAI_CREATIVE_QA_MODEL',
    default='gpt-5.6-sol',
)

OPENAI_IMAGE_MODEL = config(
    'OPENAI_IMAGE_MODEL',
    default='gpt-image-2',
)

OPENAI_VIDEO_MODEL = config(
    'OPENAI_VIDEO_MODEL',
    default='sora-2',
)
...
MARKET_RESEARCH_MAX_RESULTS = config(
    'MARKET_RESEARCH_MAX_RESULTS',
    default=8,
    cast=int,
)

SHOPPING_AGENT_MAX_SMART_SOURCES = config(
    'SHOPPING_AGENT_MAX_SMART_SOURCES',
    default=6,
    cast=int,
)

SHOPPING_AGENT_IMMERSIVE_PRODUCT_LIMIT = config(
    'SHOPPING_AGENT_IMMERSIVE_PRODUCT_LIMIT',
    default=5,
    cast=int,
)
...
# Şifre sadece environment'tan gelir.
...
CONTACT_TO_EMAIL = config(
    'CONTACT_TO_EMAIL',
    default=CONTACT_FROM_EMAIL,
)

DEMO_REQUEST_FROM_EMAIL = config(
    'DEMO_REQUEST_FROM_EMAIL',
    default=DEFAULT_FROM_EMAIL,
)

DEMO_REQUEST_TO_EMAIL = config(
    'DEMO_REQUEST_TO_EMAIL',
    default=DEMO_REQUEST_FROM_EMAIL,
)
...
WHATSAPP_MESSAGE = config(
    "WHATSAPP_MESSAGE",
    default=(
        "Merhaba, ReklamAnaliz.net "
        "hakkinda bilgi almak istiyorum."
    ),
)
...
SITE_URL = config(
    "SITE_URL",
    default="https://www.reklamanaliz.net",
).rstrip("/")

PUBLIC_MEDIA_BASE_URL = config(
...
ACCOUNT_EMAIL_VERIFICATION_BY_CODE_ENABLED = config(
...
MFA_TRUST_ENABLED = config(
    "MFA_TRUST_ENABLED",
    default=True,
    cast=bool,
)

MFA_TRUST_COOKIE_AGE = config(
    "MFA_TRUST_COOKIE_AGE",
    default=60 * 60 * 24 * 30,
    cast=int,
)
...
MFA_PASSKEY_SIGNUP_ENABLED = config(
...
ACCOUNT_RATE_LIMITS = {
    "login": config(
        "ACCOUNT_RATE_LIMIT_LOGIN",
        default="30/m/ip",
    ),

    "login_failed": config(
        "ACCOUNT_RATE_LIMIT_LOGIN_FAILED",
        default="10/m/ip,5/300s/key",
    ),

    "signup": config(
        "ACCOUNT_RATE_LIMIT_SIGNUP",
        default="10/m/ip",
    ),

    "reset_password": config(
        "ACCOUNT_RATE_LIMIT_RESET_PASSWORD",
        default="20/m/ip,5/m/key",
    ),

    "reset_password_from_key": config(
        "ACCOUNT_RATE_LIMIT_RESET_PASSWORD_FROM_KEY",
        default="20/m/ip",
    ),

    "change_password": config(
        "ACCOUNT_RATE_LIMIT_CHANGE_PASSWORD",
        default="5/m/user",
    ),

    "manage_email": config(
        "ACCOUNT_RATE_LIMIT_MANAGE_EMAIL",
        default="10/m/user",
    ),
}
...
RATE_LIMIT_CONTROL_TOWER_AI = config(
    "RATE_LIMIT_CONTROL_TOWER_AI",
    default="6/h",
)

TAVILY_MONTHLY_LIMIT = config(
    "TAVILY_MONTHLY_LIMIT",
    default=1000,
    cast=int,
)

TAVILY_RATE_LIMIT = config(
    "TAVILY_RATE_LIMIT",
    default="100/m",
)
...
# ÖNEMLİ:
#
# Redis şifresi settings.py içinde YOKTUR.
#
# Canlı sistem:
#
# REDIS_URL
...
# REDIS_PASSWORD
#
# .env / Dokploy Environment Variables'dan gelir.
#
# Redis DB:
#   0 = Celery Broker
...
def _redis_url_for_db(
    base_url: str,
    db: int,
) -> str:
    """
    Redis URL içindeki authentication bilgilerini
    koruyarak logical database numarasını değiştirir.
    """
...
if not DEBUG and not REDIS_URL:
    if REDIS_HOST == "127.0.0.1":
        raise RuntimeError(
            "Production Redis configuration is missing. "
            "Set REDIS_URL or REDIS_HOST in Dokploy "
...
AI_ANALYSIS_ENABLED = config(
    'AI_ANALYSIS_ENABLED',
    default=True,
    cast=bool,
)

AI_AUTO_REPORT = config(
    'AI_AUTO_REPORT',
    default=True,
    cast=bool,
)

AI_CREDITS_ENFORCED = config(
    "AI_CREDITS_ENFORCED",
    default=True,
    cast=bool,
)

TRIAL_ENABLED = config(
    "TRIAL_ENABLED",
    default=True,
    cast=bool,
)
...
TRIAL_AI_CREDITS = config(
    "TRIAL_AI_CREDITS",
    default=50,
    cast=int,
)

SUBSCRIPTION_ACCESS_ENFORCED = config(
    "SUBSCRIPTION_ACCESS_ENFORCED",
    default=True,
    cast=bool,
)
...
CELERY_BROKER_URL = config(
    "CELERY_BROKER_URL",
    default=_redis_url(0),
)

CELERY_RESULT_BACKEND = config(
    "CELERY_RESULT_BACKEND",
    default=_redis_url(1),
)

CHANNEL_REDIS_URL = config(
    "CHANNEL_REDIS_URL",
    default=_redis_url(2),
)
...
CELERY_TASK_ROUTES = {
    ...
    "core.tasks.v2_platform_sync.*": {
        "queue": "sync"
    },

    "core.tasks.platform_tasks.*": {
        "queue": "sync"
    },
    ...
    "core.tasks.admin_ops.dispatch_admin_managed_schedules": {
        "queue": "maintenance"
    },

    "core.tasks.admin_ops.refresh_expired_tokens": {
        "queue": "maintenance"
    },

    "core.tasks.admin_ops.refresh_due_marketplace_researches": {
        "queue": "marketplace"
    },

    "core.tasks.admin_ops.generate_octo_tasks": {
        "queue": "ai"
    },

    "core.tasks.admin_ops.dispatch_octo_rule_engine_sweep": {
        "queue": "ai"
    },
}
...
CELERY_BEAT_SCHEDULE = {
    ...
    "dispatch-octo-rule-engine-sweep": {
        "task":
            "core.tasks.admin_ops."
            "dispatch_octo_rule_engine_sweep",
    ...
    "scan-critical-alerts": {
        "task":
            "core.tasks.notification_tasks."
            "scan_critical_alerts_for_all_users",
    ...
    "send-daily-notification-summaries": {
        "task":
            "core.tasks.notification_tasks."
            "send_daily_notification_summaries",
    ...
    "record-daily-metrics": {
        "task":
            "core.tasks.metric_tasks."
            "record_daily_metrics_for_all_ads",
    ...
    "purge-expired-pending-deletion-accounts": {
        "task":
            "core.tasks.maintenance_tasks."
            "sync_account_deletion_lifecycle",
    ...
    "refresh-tracked-marketplace-researches": {
        "task":
            "core.tasks.marketplace_sync."
            "refresh_tracked_marketplace_researches",
    ...
    "dispatch-admin-managed-schedules": {
        "task":
            "core.tasks.admin_ops."
            "dispatch_admin_managed_schedules",
...
REFERRAL_PROGRAM_ENABLED = os.getenv(
